...
function DocumentResult({ doc, onClose, onApprove, onReject }) {
  ...
  // Helper — parse "Row N: FieldName" into grouped employee records
  const buildEmployeeRecords = () => {
    const rows = {};
    Object.entries(fields).forEach(([key, val]) => {
      ...
      if (match) {
        ...
        const field  = match[2];
        if (!rows[rowNum]) rows[rowNum] = { Employee: rowNum };
        rows[rowNum][field] = val || "";
      }
    });
  ...
  const handleExportCSV = () => {
    ...
    if (records.length > 0) {
      // Employee table format — one row per employee
      const cols = ["Employee", ...Object.keys(records[0]).filter(k => k !== "Employee")];
      const header = cols.join(",");
      const dataRows = records.map(r => cols.map(c => `"${r[c] ?? ""}"`).join(","));
      const csv = [header, ...dataRows].join("\n");
      download(csv, `${baseName}_results.csv`, "text/csv");
    } else {
      // Generic flat format
      const csv = [
        "Field,Value",
  ...
  const handleExportJSON = () => {
    ...
    const output = {
      ...
      recordId,
      confidence: (avgConfidence ?? confidence) != null ? ((avgConfidence ?? confidence) * 100).toFixed(1) + "%" : "—",
      status:     doc.result?.status || "completed",
      humanReviewed: validated,
      employeeRecords: records.length > 0 ? records : fields,
    };
    download(JSON.stringify(output, null, 2), `${baseName}_results.json`, "application/json");
  };
  ...
  const handleExportTXT = () => {
    ...
    if (records.length > 0) {
      lines.push(`EMPLOYEE RECORDS (${records.length} employees)`);
      lines.push(divider);
      records.forEach((rec, i) => {
        lines.push(`Employee ${i + 1}`);
    ...
    lines.push(divider);
    lines.push("Generated by DocIntel · Azure Content Understanding");
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <button onClick={handleExportCSV} style={{ padding: "7px 14px", borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: "pointer", border: "1px solid #e2e8f0", background: "#fff", color: "#334155" }}>↓ CSV</button>
              <button onClick={handleExportJSON} style={{ padding: "7px 14px", borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: "pointer", border: "1px solid #e2e8f0", background: "#fff", color: "#334155" }}>↓ JSON</button>
              <button onClick={handleExportTXT} style={{ padding: "7px 14px", borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: "pointer", border: "1px solid #e2e8f0", background: "#fff", color: "#334155" }}>↓ TXT</button>
            </div>
            {!validated && (
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "#166534", marginTop: 4 }}>
                    Record ID: <strong>{recordId}</strong> &nbsp;·&nbsp; {Object.keys(fields).length} fields &nbsp;·&nbsp; {new Date().toLocaleString()}
                  </p>
                </div>
                <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                  <button onClick={handleExportCSV} style={{ padding: "6px 13px", borderRadius: 7, border: "1px solid #86efac", background: "#fff", color: "#15803d", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                    ↓ CSV
                  </button>
                  <button onClick={handleExportJSON} style={{ padding: "6px 13px", borderRadius: 7, border: "1px solid #86efac", background: "#fff", color: "#15803d", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                    ↓ JSON
                  </button>
                  <button onClick={handleExportTXT} style={{ padding: "6px 13px", borderRadius: 7, border: "1px solid #86efac", background: "#fff", color: "#15803d", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                    ↓ TXT
                  </button>
                </div>
              </div>
